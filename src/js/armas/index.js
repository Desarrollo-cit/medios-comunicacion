...
let tablaProductos = new Datatable('#armasTabla');
...
const guardararmas = async (evento) => {
    evento.preventDefault();

    let formularioValido = validarFormulario(formArmas, ['id']);
    if (!formularioValido) {
        Toast.fire({
            icon: 'warning',
    ...
    try {
        ...
        const url = '/medios-comunicacion/API/armas/guardar'
...
const buscararmas = async (evento) => {
    evento && evento.preventDefault();

    try {
        const url = '/medios-comunicacion/API/armas/buscar'
        const headers = new Headers();
        headers.append("X-Requested-With", "fetch");
        ...
        let contador = 1;
        tablaProductos = new Datatable('#armasTabla', {
            language : lenguaje,
            data : data,
            columns : [
                { 
...
const modificararmas = async (evento) => {
    evento.preventDefault();

    let formularioValido = validarFormulario(formArmas);
    if (!formularioValido) {
        Toast.fire({
            icon: 'warning',
            title: 'Debe llenar todos los campos'
    ...
    try {
        //Crear el cuerpo de la consulta
        const url = '/medios-comunicacion/API/armas/modificar'
...
window.eliminarRegistro = (id) => {
    Swal.fire({
        ...
        if(result.isConfirmed){
            const url = '/medios-comunicacion/API/armas/eliminar'
...
window.cambiarSituacion = (id, situacion, desc) => {
    ...
    Swal.fire({
        ...
        if(result.isConfirmed){
            const url = '/medios-comunicacion/API/armas/cambiarSituacion'
            const body = new FormData();
            body.append('id', id);
